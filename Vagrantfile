...
Vagrant.configure("2") do |config|
  ...
  config.vm.hostname = "AToth"
  
  # Comparto la carpeta del host donde estoy parado contra la vm
  config.vm.synced_folder '.', '/home/vagrant/carpeta_compartida'
  
  # Configuración de Red
  config.vm.network "private_network", :name => '', ip: "192.168.56.35", virtualbox__intnet: true

  # Agrego disco rigidos Extras (Nombres distintos)
  config.vm.disk :disk, size: "3GB", name: "discos_1"
  config.vm.disk :disk, size: "1GB", name: "discos_2"
  config.vm.disk :disk, size: "1GB", name: "discos_3"
